...
2. Enable the team-repo pre-commit hook (needs `python3`):
   ...
   The agent shell guard (`.cursor/hooks.json`) needs `jq` and loads automatically.
...
Definition of Ready (needed for BACKLOG → READY): `parent` set, `assignee` set, ≥1 real acceptance
criterion, `depends_on` filled (or `[]`), Design filled or linked.

## Stack

See `project.md` and `docs/adr/0003-product-tech-stack.md`.

| Area | Choice |
|------|--------|
| Backend | Java 21, Spring Boot 4.0.8, Maven wrapper, PostgreSQL + Flyway |
| Frontend | React + TypeScript, Vite, TanStack Query, oxlint + Prettier |
...
| `docs/standards/` | BE / FE / testing standards agents must follow |
...
| `templates/` | Requirement, task, design, sprint |
